public class Node {
  ...
  public Node getAdjNodeByAngle(float angle, float len) {
    Node bestNode = null;
    float minDiff = radians(30);
    for (Map.Entry < Node, Integer> adjacencyPair : getAdjacentNodes().entrySet()) {
      Node adjacentNode = adjacencyPair.getKey();
      float adjAngle = getAngle(this, adjacentNode);
      float distanceBetweenPoints = getPointDistance(this, adjacentNode);
      if (abs(angle-adjAngle) < minDiff && distanceBetweenPoints < len + 50 && distanceBetweenPoints > len - 50) {
        bestNode = adjacentNode;
        minDiff = abs(angle-adjAngle);
      }
    }
    return bestNode;
  }
  
  public float getAngle(Node start, Node end) {
